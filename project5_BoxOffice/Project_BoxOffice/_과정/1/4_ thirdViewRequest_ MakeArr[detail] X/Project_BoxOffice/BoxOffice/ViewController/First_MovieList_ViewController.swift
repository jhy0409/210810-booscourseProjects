...
class First_MovieList_ViewController: UIViewController {
    ...
    @objc func didRiecieveMovieNotification(_ noti: Notification) {
        ...
        if movies.count != 0 {
            let arr = shared.movieList?.makeMovieDetailArr()
            
            if let aaaaa = arr {
                print("\n\n\n🧝‍♀️🧝‍♀️🧝‍♀️🧝‍♀️arr count = \(aaaaa.count)")
                shared.movieDetailArr = aaaaa
            }
            
            print("\n\n\n🧝‍♀️🧝‍♀️🧝‍♀️🧝‍♀️🧝‍♀️🧝‍♀️🧝‍♀️🧝‍♀️🧝‍♀️🧝‍♀️arr count = \(arr?.count)")
        }
        
        
//        shared.movieList?.makeMovieDetailArr()
