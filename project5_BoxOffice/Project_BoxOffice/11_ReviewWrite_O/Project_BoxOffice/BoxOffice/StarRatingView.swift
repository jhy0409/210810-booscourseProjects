...
//    fileprivate let halfStarImage: UIImage = UIImage(systemName: "ic_star_large_half")!
     let fullStarImage: UIImage = UIImage(named: "ic_star_large_full")!
     let halfStarImage: UIImage = UIImage(named: "ic_star_large_half")!
     let emptyStarImage: UIImage = UIImage(named: "ic_star_large")!
...
//    fileprivate func setStarsFor(rating: Float) {
    func setStarsFor(rating: Float)  {
        ...
        for i in 1...5 {
            let iFloat = Float(i)
            StarRatingView.resultSum += iFloat
            switch starRounding {
...
//    }
    ...
    fileprivate func touched(touch: UITouch, moveTouch: Bool) {
        ...
        StarRatingView.resultSum = self.rating
        print("\n\n---> 🏆🏆🏆🏆🏆 StarRatingView.resultSum : \(StarRatingView.resultSum)")
        lastTouch = Date()
    }
