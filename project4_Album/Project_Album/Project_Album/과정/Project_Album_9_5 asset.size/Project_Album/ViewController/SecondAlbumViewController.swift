...
class SecondAlbumViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        // MARK: - [] 👹👹👹👹 어셋 이미지 사이즈
        //imageManager.requestImage(for: assets[indexPath.item], targetSize: cell.photoImgView.bounds.size, contentMode:
        let size: CGSize = CGSize(width: asset.pixelWidth, height: asset.pixelHeight)
        print("\n\n👹👹 size.width : \(size.width), size.height: \(size.height)\n👹👹 cell.bounds.size: \(cell.bounds.size)")
        imageManager.requestImage(for: assets[indexPath.item], targetSize: size, contentMode:                                     .aspectFill, options: nil) { image, _  in
